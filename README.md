...
- If you want to change the game port, you can add `-port <port>` to `EXTRA_PARAMS` and then update your router / cloud security list / host firewall rules to the matching CS2 port set. If you run non-default ports, verify the full required mapping (including related ports such as SourceTV/client ports) in current CS2 server docs.
